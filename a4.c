...
{	
	...
	snd_pcm_readi (p_capture_handle, ap_rec_buffer, 160); 
	//160000/16000 = 10
	printf("녹음이 완료 되었습니다.\n");
...
void PlaySound(unsigned char *ap_play_buffer)
{
	...
	snd_pcm_writei(p_playback_handle, ap_play_buffer, 160);
	usleep(100000); // wait... 10 sec = 10000000
	printf("재생을 완료 합니다.\n");
...
{ 
	while(1){
		unsigned char rec_buffer[640];
		if(RecordSound(rec_buffer))PlaySound(rec_buffer);
	}	
		return 0;
}
